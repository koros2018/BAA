...
|--------|------|------|
| ~~P7-P40~~ | 已完成 | ✅ |
| ~~P119~~ | ~~违规审核工作流（确认/误报/待核实）~~ | ✅ **2026-09-07 完成**（`bd4c01d`，runReview TS 化 + audit 面板闭环 + 整改通知单下载，46 测试全绿） |
| ~~P120~~ | ~~异常处理全量修复（pass空吞/裸except）~~ | ✅ **2026-08-19 完成**（`89f44ca`，57 处 pass 清零 + 裸 except 1 处） |
| ~~P121~~ | ~~大文件解析（结构图SKIP44%+大图SKIP18%）~~ | ✅ **2026-08-22 完成**（`386b6b5` 分类器 + `2ea0c13` API 分类 + `d7fab45` timeout+partial） |
...
| P127 | 天正 T3 降级管线（L1 补齐） | ✅ 2026-09-11 完成（`3e79bf6`，三层降级分类管线 + 42 单测全绿 + 端到端扫描脚本；东莞通 DXF 实测 34748 构件 wall/door/window/column/text 分类正确；T3 加密文件 0 图元已加 parse_status 诊断，识别率验收闭环见 P135） |
...
| P131 | IFC/STEP 反向重构输出（L4） | 🆕 新增（IfcOpenShell + CadQuery，Hausdorff≤2mm） |
| P132 | 闭环量化指标（L5） | 🆕 新增（Hausdorff / 图编辑距离 / 语义匹配 / 参数保真，偏差≤5%） |
| P133 | 数据合成完整管线 | 🆕 新增（参数化 + 拓扑保持 + 样式随机化 + 噪声注入，500 合成 + 300 真实） |
| P134 | 许可证合规隔离 | 🆕 新增（AGPL/GPL 组件独立微服务/独立进程） |
| P135 | 天正 T3 识别率验收闭环（L1 量化） | 🆕 新增（P127 的「墙≥80%」缺分母，无标注集无法算 recall。三段式：① **标注集构建** — 用 AutoCAD 执行 T3→T0 另存 DXF，人工标注 ground truth，覆盖 ≥3 张图纸、墙/门/窗/柱 4 类、每张 ≥50 实体；② **基线测量** — P127 三层降级管线跑标注集，产出各类 precision/recall/F1；③ **差距分析** — 定位图层命名规范覆盖率瓶颈，判定是否需补 XData 探测或几何聚类增强。验收：墙 F1≥0.80、门窗 F1≥0.75，输出标注集 + 测量报告 + 瓶颈结论。附带核实：天正官网无公开免费识别 API（4 组检索未命中，二次开发/云 API 生态均指向 Autodesk APS 而非天正）） |
| P136 | 扫描件 OCR 接入（L2 补齐-阻塞中） | ⛔ 阻塞（P128 Phase 2 拆出。需真实扫描件样本才能度量「扫描件≥85%」，当前 data/ 内无任何扫描件：drawings/real/ 为 10 DXF + 1 矢量 PDF，real_renderings/ 的 JPEG 是 DXF 渲染图供 YOLO 用，coco/train/ 是 508×508 合成图。前置条件：① 拿到真实图纸 PDF/扫描图；② 装 PyMuPDF + pytesseract（已装 1.28.2 / 0.3.13）；③ 用 pdftoppm 渲染降级造样本集，实测 tesseract chi_sim 在图纸标注文字上的准确率后再定门槛。技术选型已定：用 pytesseract 而非 PaddleOCR（系统已装 tesseract 5.3.4 + chi_sim 语言包，PaddleOCR 需额外 1GB+ paddle 且 Python 3.12 兼容麻烦）） |
| **📌 v2 路线图** | `docs/07-重大变更/工程可行性研究报告及设计（终稿）/baa-roadmap-20260909-v2.md` | 基于两份数据中心图纸方案对齐（2026-09-09） |
...
| ~~P114~~ | ~~前端架构重构（含E2E测试）~~ | ✅ 已被 P122/P123 覆盖完成 |
| P115 | 云端部署流水线与生产配置 | 🟢 后续 |
| P117 | 模型参数、规范版本与知识库开放 | 🟢 后续 |
| P118 | 商业模式与产品定位验证 | 🟢 后续 |
| ~~P111~~ | ~~YOLOv8m 训练~~ | ❌ **已证伪，终止** |

### v2.5.76-stable 发布记录
- **发布日**：2026-09-07
- **HEAD**：`3842496`
- **P 项完成**：P119（违规审核工作流 + 整改通知单）、P120（异常处理）、P121（大文件解析）、P122（前端安全）、P123（前端 Vite+TS 重构）
- **前端**：Vite + TS 全面迁移（30+ 组件 / 13 core 模块），46 测试全绿，全量 2196 passed

### v2.5.77-stable 发布记录
- **发布日**：2026-09-09
- **HEAD**：`b77c69b`
- **P 项完成**：P124（前端遗留清理 + E2E 补全）
- **遗留清零**：onclick 87→0 / innerHTML+= 6→0 / var 2→0
