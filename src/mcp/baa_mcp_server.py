...
class BAAMCPServer:
                    ...
                    inputSchema={
                        ...
                        "properties": {
                            ...
                            "options": {
                                ...
                                "description": "重构参数（可选）",
                                "properties": {
                                    "lod": {"type": "integer", "description": "LOD: 100/200/300"},
                                    "format": {
                                        "type": "string",
                ...
                Tool(
                    ...
                    inputSchema={
                        ...
                        "properties": {
                            "category": {
                                "type": "string",
                                "description": "筛选类别: dim / dist / exist / count / area / attr / access / evac",
                            },
                        },
                        "required": [],
                    },
        ...
        async def list_resources() -> list[Resource]:
            return [
                Resource(
                    ...
                    name="原子函数总数",
                    description=f"当前注册原子函数数: {self._get_func_count()} / 260",
                    mimeType="application/json",
                ),
                Resource(
                    uri="baa://specs/list",
                    name="已加载规范列表",
                    description="BAA 支持的所有建筑规范",
                    mimeType="application/json",
                ),
            ]
        ...
        async def read_resource(uri: str) -> ReadResourceContents:
            ...
            if uri == "baa://functions/count":
                return ReadResourceContents(
                    content=json.dumps(
                        {
                            "count": self._func_registry.count,
                            "capacity": self._func_registry.capacity,
                        },
                        ensure_ascii=False,
                    ),
        ...
        async def call_tool(name: str, arguments: dict) -> list[TextContent]:
            try:
                handlers = {
                    ...
                    "baa_review": self._handle_review,
                    "baa_reconstruct": self._handle_reconstruct,
                    "baa_health": self._handle_health,
                ...
                return [
                    TextContent(
                        type="text",
                        text=json.dumps(
                            {"status": "error", "error_code": type(e).__name__, "message": str(e)},
                            ensure_ascii=False,
                        ),
                    )
                ]
    ...
    async def _handle_deconstruct(self, args: dict) -> dict:
        ...
        for e in entities:
            ...
            type_stats[t]["bbox_areas"].append(
                e["bbox"].get("width", 0) * e["bbox"].get("height", 0)
            )
        elements = []
        ...
        return {
            ...
            "confidence": 0.85 if entities else 0,
            "file_id": file_id,
    ...
    async def _handle_review(self, args: dict) -> dict:
        ...
        findings, total_checks = await loop.run_in_executor(
            self._thread_pool, self._do_clustering, entities, building_type
        )
        ...
        return {
            ...
            "summary": {
                ...
                "violation_by_clause": dict(
                    Counter(f["clause_id"] for f in findings).most_common(10)
                ),
            },
