...
class AtomicFunction:
    ...
    def _extract_value(self, entity: Dict[str, Any]) -> float:
        ...
        if func_id == "ATTR-001":  # 防火门等级
            ...
            has_fire_rating = "fire_rating" in props or "rating" in props
        ...
        if func_id == "DIM-009":  # 疏散出口宽度
            ...
            entity_type = entity.get("type", "")
            if entity_type not in ("exit", "exit_door") and val < 1.3:
        ...
        if func_id == "AREA-002":  # 消防电梯前室面积
            ...
            if val < 0.01:
                ...
                if bw > 0 and bh > 0:
                    ...
                    bbox_area_m2 = bbox_area / 1000000.0
                    if 0.5 <= bbox_area_m2 <= 500:
                        val = bbox_area
                    else:  # 否则
...
class FuncRegistry:
    ...
    INITIAL_FUNCS = [
        ...
        AtomicFunction("EXIST-005", "自动灭火系统判定", FuncCategory.EXIST,
                       ...
                       target_entities=["sprinkler_system", "sprinkler", "fire_hydrant", "fire_extinguisher", "fire_system"]),
