...
class DrawingParser:  # class definition
        ...
        if parent_dir.exists() and parent_dir != path.parent:  # check: AND condition
            ...
            for dxf_file in parent_dir.rglob("*.dxf"):  # loop: iterate
                ...
                if self._names_match(
                    dwg_name, dxf_stem
                ):  # condition: self._names_match(dwg_name, dxf_stem):
                    try:  # try block
    ...
    def _names_match(
        self, dwg_name: str, dxf_name: str
...
"""
    ...
    def _compute_bbox(
        self, entity
    ) -> Dict[str, float]:  # method: def _compute_bbox(self, entity) -> Dict[str, float]:
        """计算图元边界框
        ...
        try:  # 尝试
            if hasattr(entity, "bbox"):  # condition: hasattr(entity, 'bbox'):
                ...
                if (
                    bbox and bbox.extmin is not None and bbox.extmax is not None
    ...
    def _extract_properties(
        self, entity
    ) -> Dict[str, Any]:  # method: def _extract_properties(self, entity) -> Dict[str, Any]:
        """提取几何属性"""
        props = {}  # assignment
    ...
    def _compute_polygon_area(
        entity,
    ) -> float:  # method: def _compute_polygon_area(entity) -> float:
        """计算多边形面积"""
        try:  # 尝试
            points = list(entity.vertices())  # list conversion
            ...
            area = 0.5 * abs(
                sum(
                    xs[i] * ys[i + 1] - xs[i + 1] * ys[i]  # assignment
