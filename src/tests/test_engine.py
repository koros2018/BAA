...
from src.baa_engine.semantic_analyzer import (
    SemanticAnalyzer,
    SemanticEntity,
    SpatialRelation,
)  # import
from src.baa_engine.drawing_parser import RawPrimitive  # import
from src.baa_engine.cache import (
    PersistentCache,
    make_cache_key,
    make_drawing_cache_key,
...
class TestFuncRegistry:  # class definition
    ...
    def test_initial_count(self):  # function: def test_initial_count(self):
        ...
        assert registry.count == 406  # P45 新增 thermal 16 条（GB50176/GB50189/GB55015 热工性能）
        assert registry.capacity == 406  # P45 新增 thermal 16 条后框架总容量（294 + 96 余量 + 16）
    ...
    def test_list_all(self):  # function: def test_list_all(self):
        ...
        assert len(all_funcs) == 406  # P45 新增 thermal 16 条
        categories = set(f.category for f in all_funcs)  # function call
        for cat in [
            FuncCategory.DIMENSION,
            FuncCategory.DISTANCE,  # 循环
            FuncCategory.COUNT,
            FuncCategory.ATTR,  # 解包
            FuncCategory.AREA,
            FuncCategory.EXIST,  # 解包
            FuncCategory.EVAC,
        ]:  # 操作
    ...
    def test_register_duplicate_does_not_increase_count(
        self,
    ):  # function: def test_register_duplicate_does_not_increase_count(self):
        registry = FuncRegistry()  # function call
        ...
        dupe = AtomicFunction(  # assignment
            ...
            description="测试",
            category=FuncCategory.DIMENSION,  # assignment
            target_entities=["staircase"],
            operator=">=",
            threshold=1.2,
            unit="m",  # assignment
        )  # code
    ...
    def test_register_up_to_capacity(self):  # function: def test_register_up_to_capacity(self):
        ...
        for i in range(remaining):  # 循环
            func = AtomicFunction(  # assignment
                func_id=f"TEST-{i:03d}",
                name=f"测试{i}",
                clause_id="TEST",  # assignment
                description="测试",
                category=FuncCategory.DIMENSION,  # assignment
                target_entities=["wall"],
                operator=">=",
                threshold=1.0,
                unit="m",  # assignment
            )  # code
...
class TestFuncExecute:  # class definition
    ...
    def test_dim001_pass(self, registry):  # function: def test_dim001_pass(self, registry):
        ...
        r = func.execute(
            {"id": "S1", "type": "staircase", "properties": {"clear_width": 1.30}}
        )  # function call
    ...
    def test_dim001_fail(self, registry):  # function: def test_dim001_fail(self, registry):
        func = registry.get("DIM-001")  # function call
        r = func.execute(
            {"id": "S2", "type": "staircase", "properties": {"clear_width": 1.05}}
        )  # function call
    ...
    def test_dim001_boundary(self, registry):  # function: def test_dim001_boundary(self, registry):
        func = registry.get("DIM-001")  # function call
        r = func.execute(
            {"id": "S3", "type": "staircase", "properties": {"clear_width": 1.20}}
        )  # function call
        assert r.result == "PASS"  # 断言

    def test_dim001_wrong_entity(
        self, registry
    ):  # function: def test_dim001_wrong_entity(self, registry):
        func = registry.get("DIM-001")  # function call
        r = func.execute(
            {"id": "D1", "type": "door", "properties": {"clear_width": 1.05}}
        )  # function call
        assert r is None  # 断言
    ...
    def test_dim002_civil_pass(
        ...
        r = func.execute(
            {"id": "FZ1", "type": "fire_zone", "properties": {"area": 2000.0}}
        )  # function call
        assert r.result == "PASS"  # 断言

    def test_dim002_civil_fail(
        self, registry
    ...
    def test_dim003_pass(self, registry):  # function: def test_dim003_pass(self, registry):
        r = registry.get("DIM-003").execute(
            {"id": "FL1", "type": "fire_lane", "properties": {"width": 4.5}}
        )  # function call
        assert r.result == "PASS"  # 断言

    def test_dim003_fail(self, registry):  # function: def test_dim003_fail(self, registry):
        r = registry.get("DIM-003").execute(
            {"id": "FL2", "type": "fire_lane", "properties": {"width": 3.0}}
        )  # function call
        assert r.result == "FAIL"  # 断言

    # DIM-004: 疏散走道宽度
    def test_dim004_civil_pass(
        self, registry
    ):  # function: def test_dim004_civil_pass(self, registry):
        func = registry.get("DIM-004")  # function call
        func.threshold = 1.1  # assignment
        r = func.execute(
            {"id": "C1", "type": "corridor", "properties": {"clear_width": 1.4}}
        )  # function call
        assert r.result == "PASS"  # 断言

    def test_dim004_industrial_fail(
        self, registry
    ):  # function: def test_dim004_industrial_fail(self, registry):
        func = registry.get("DIM-004")  # function call
        func.threshold = 1.4  # assignment
        r = func.execute(
            {"id": "C2", "type": "corridor", "properties": {"clear_width": 1.0}}
        )  # function call
        assert r.result == "FAIL"  # 断言
    ...
    def test_dim005_pass(self, registry):  # function: def test_dim005_pass(self, registry):
        r = registry.get("DIM-005").execute(
            {"id": "FW1", "type": "fire_window", "properties": {"area": 2.0}}
        )  # function call
        assert r.result == "PASS"  # 断言

    def test_dim005_fail(self, registry):  # function: def test_dim005_fail(self, registry):
        r = registry.get("DIM-005").execute(
            {"id": "FW2", "type": "fire_window", "properties": {"area": 0.5}}
        )  # function call
        assert r.result == "FAIL"  # 断言

    # DIM-006: 疏散门净宽
    def test_dim006_civil_pass(
        self, registry
    ):  # function: def test_dim006_civil_pass(self, registry):
        func = registry.get("DIM-006")  # function call
        func.threshold = 1.4  # assignment
        r = func.execute(
            {"id": "ED1", "type": "exit_door", "properties": {"clear_width": 1.5}}
        )  # function call
        assert r.result == "PASS"  # 断言

    def test_dim006_industrial_pass(
        self, registry
    ):  # function: def test_dim006_industrial_pass(self, registry):
        func = registry.get("DIM-006")  # function call
        func.threshold = 1.2  # assignment
        r = func.execute(
            {"id": "ED2", "type": "exit_door", "properties": {"clear_width": 1.3}}
        )  # function call
        assert r.result == "PASS"  # 断言

    # DIM-007: 防火卷帘宽度 (<= 10)
    def test_dim007_pass(self, registry):  # function: def test_dim007_pass(self, registry):
        r = registry.get("DIM-007").execute(
            {"id": "FC1", "type": "fire_curtain", "properties": {"width": 8.0}}
        )  # function call
        assert r.result == "PASS"  # 断言

    def test_dim007_fail(self, registry):  # function: def test_dim007_fail(self, registry):
        r = registry.get("DIM-007").execute(
            {"id": "FC2", "type": "fire_curtain", "properties": {"width": 12.0}}
        )  # function call
        assert r.result == "FAIL"  # 断言

    # DIST-001: 疏散距离
    def test_dist001_civil_pass(
        self, registry
    ):  # function: def test_dist001_civil_pass(self, registry):
        func = registry.get("DIST-001")  # function call
        func.threshold = 30.0  # assignment
        r = func.execute(
            {"id": "R1", "type": "room", "properties": {"travel_distance": 20.0}}
        )  # function call
        assert r.result == "PASS"  # 断言

    def test_dist001_industrial_fail(
        self, registry
    ):  # function: def test_dist001_industrial_fail(self, registry):
        func = registry.get("DIST-001")  # function call
        func.threshold = 40.0  # assignment
        r = func.execute(
            {"id": "R2", "type": "room", "properties": {"travel_distance": 50.0}}
        )  # function call
        assert r.result == "FAIL"  # 断言

    # COUNT-001: 安全出口数量
    def test_count001_pass(self, registry):  # function: def test_count001_pass(self, registry):
        r = registry.get("COUNT-001").execute(
            {"id": "F1", "type": "floor", "properties": {"exit_count": 3}}
        )  # function call
        assert r.result == "PASS"  # 断言

    def test_count001_fail(self, registry):  # function: def test_count001_fail(self, registry):
        r = registry.get("COUNT-001").execute(
            {"id": "F2", "type": "floor", "properties": {"exit_count": 1}}
        )  # function call
        assert r.result == "FAIL"  # 断言

    # ATTR-001: 防火门等级
    def test_attr001_pass(self, registry):  # function: def test_attr001_pass(self, registry):
        r = registry.get("ATTR-001").execute(
            {"id": "FD1", "type": "fire_door", "properties": {"fire_rating": 1.0}}
        )  # function call
        assert r.result == "PASS"  # 断言

    def test_attr001_fail(self, registry):  # function: def test_attr001_fail(self, registry):
        r = registry.get("ATTR-001").execute(
            {"id": "FD2", "type": "fire_door", "properties": {"fire_rating": 0.0}}
        )  # function call
        assert r.result == "FAIL"  # 断言

    # ATTR-002: 保温材料
    def test_attr002_civil_pass(
        self, registry
    ):  # function: def test_attr002_civil_pass(self, registry):
        func = registry.get("ATTR-002")  # function call
        func.threshold = 2.0  # assignment
        r = func.execute(
            {"id": "I1", "type": "insulation", "properties": {"fire_rating": 2.0}}
        )  # function call
        assert r.result == "PASS"  # 断言

    def test_attr002_industrial_fail(
        self, registry
    ):  # function: def test_attr002_industrial_fail(self, registry):
        func = registry.get("ATTR-002")  # function call
        func.threshold = 3.0  # assignment
        r = func.execute(
            {"id": "I2", "type": "insulation", "properties": {"fire_rating": 2.0}}
        )  # function call
        assert r.result == "FAIL"  # 断言

    # AREA-001: 避难层面积 (>= 5.0)
    def test_area001_pass(self, registry):  # function: def test_area001_pass(self, registry):
        r = registry.get("AREA-001").execute(
            {"id": "RF1", "type": "refuge_floor", "properties": {"area": 6.0}}
        )  # function call
        assert r.result == "PASS"  # 断言

    def test_area001_fail(self, registry):  # function: def test_area001_fail(self, registry):
        r = registry.get("AREA-001").execute(
            {"id": "RF2", "type": "refuge_floor", "properties": {"area": 3.0}}
        )  # function call
        assert r.result == "FAIL"  # 断言

    # LIGHT-001: 应急照明
    def test_light001_pass(self, registry):  # function: def test_light001_pass(self, registry):
        r = registry.get("LIGHT-001").execute(
            {"id": "EL1", "type": "evacuation_lighting", "properties": {"illuminance": 1.5}}
        )  # function call
        assert r.result == "PASS"  # 断言

    def test_light001_fail(self, registry):  # function: def test_light001_fail(self, registry):
        r = registry.get("LIGHT-001").execute(
            {"id": "EL2", "type": "evacuation_lighting", "properties": {"illuminance": 0.5}}
        )  # function call
        assert r.result == "FAIL"  # 断言
    ...
    def test_dist002_pass(self, registry):  # function: def test_dist002_pass(self, registry):
        r = registry.get("DIST-002").execute(
            {"id": "B1", "type": "building", "properties": {"distance": 15.0}}
        )  # function call
        assert r.result == "PASS"  # 断言

    def test_dist002_fail(self, registry):  # function: def test_dist002_fail(self, registry):
        r = registry.get("DIST-002").execute(
            {"id": "B2", "type": "factory", "properties": {"distance": 8.0}}
        )  # function call
        assert r.result == "FAIL"  # 断言

    # DIM-008: 排烟窗面积
    def test_dim008_pass(self, registry):  # function: def test_dim008_pass(self, registry):
        r = registry.get("DIM-008").execute(
            {"id": "SW1", "type": "smoke_exhaust_window", "properties": {"area": 0.05}}
        )  # function call
        assert r.result == "PASS"  # 断言

    def test_dim008_fail(self, registry):  # function: def test_dim008_fail(self, registry):
        r = registry.get("DIM-008").execute(
            {"id": "SW2", "type": "smoke_exhaust_window", "properties": {"area": 0.01}}
        )  # function call
        assert r.result == "FAIL"  # 断言

    # EXIST-007: 消防电梯
    def test_exist007_pass(self, registry):  # function: def test_exist007_pass(self, registry):
        r = registry.get("EXIST-007").execute(
            {"id": "FE1", "type": "fire_elevator", "properties": {"exists": True}}
        )  # function call
        assert r.result == "PASS"  # 断言

    def test_exist007_missing(
        self, registry
    ):  # function: def test_exist007_missing(self, registry):
    ...
    def test_exist009_pass(self, registry):  # function: def test_exist009_pass(self, registry):
        r = registry.get("EXIST-009").execute(
            {"id": "WR1", "type": "water_reservoir", "properties": {"exists": True}}
        )  # function call
        assert r.result == "PASS"  # 断言

    def test_exist009_missing(
        self, registry
    ):  # function: def test_exist009_missing(self, registry):
    ...
    def test_dim010_pass(self, registry):  # function: def test_dim010_pass(self, registry):
        r = registry.get("DIM-010").execute(
            {"id": "RW1", "type": "rescue_window", "properties": {"area": 1.5}}
        )  # function call
        assert r.result == "PASS"  # 断言

    def test_dim010_fail(self, registry):  # function: def test_dim010_fail(self, registry):
        r = registry.get("DIM-010").execute(
            {"id": "RW2", "type": "rescue_window", "properties": {"area": 0.5}}
        )  # function call
        assert r.result == "FAIL"  # 断言

    # EXIST-010: 应急广播
    def test_exist010_pass(self, registry):  # function: def test_exist010_pass(self, registry):
        r = registry.get("EXIST-010").execute(
            {"id": "EB1", "type": "emergency_broadcast", "properties": {"exists": True}}
        )  # function call
        assert r.result == "PASS"  # 断言

    def test_exist010_missing(
        self, registry
    ):  # function: def test_exist010_missing(self, registry):
    ...
    def test_exist001_pass(self, registry):  # function: def test_exist001_pass(self, registry):
        r = registry.get("EXIST-001").execute(
            {"id": "S1", "type": "staircase", "properties": {"exists": True, "count": 2}}
        )  # function call
        assert r.result == "PASS"  # 断言

    def test_exist001_missing(
        self, registry
    ):  # function: def test_exist001_missing(self, registry):
    ...
    def test_severity_minor(self, registry):  # function: def test_severity_minor(self, registry):
        r = registry.get("DIM-001").execute(
            {"id": "S1", "type": "staircase", "properties": {"clear_width": 1.17}}
        )  # function call
        assert r.result == "FAIL"  # 断言
    ...
    def test_severity_major(self, registry):  # function: def test_severity_major(self, registry):
        r = registry.get("DIM-001").execute(
            {"id": "S1", "type": "staircase", "properties": {"clear_width": 1.05}}
        )  # function call
        assert r.result == "FAIL"  # 断言
    ...
    def test_severity_critical(
        self, registry
    ):  # function: def test_severity_critical(self, registry):
        r = registry.get("DIM-001").execute(
            {"id": "S1", "type": "staircase", "properties": {"clear_width": 0.7}}
        )  # function call
        assert r.result == "FAIL"  # 断言
        assert r.severity == Severity.CRITICAL  # equality check
...
class TestSpecRepository:  # class definition
    ...
    def test_get_threshold_default(
        self, repo
    ):  # function: def test_get_threshold_default(self, repo):
        val, unit, op = repo.get_threshold("GB50016-5.5.18", "civil", "GB 50016-2014")  # 操作
        assert val == 1.2  # equality check
        assert unit == "m"  # 断言

    def test_get_threshold_civil_dim002(
        self, repo
    ):  # function: def test_get_threshold_civil_dim002(self, repo):
        val, _, _ = repo.get_threshold("GB50016-6.1.1", "civil", "GB 50016-2014")  # 操作
        assert val == 2500.0  # equality check

    def test_get_threshold_industrial_dim002(
        self, repo
    ):  # function: def test_get_threshold_industrial_dim002(self, repo):
        val, _, _ = repo.get_threshold("GB50016-6.1.1", "industrial", "GB 50016-2014")  # 操作
        assert val == 4000.0  # equality check

    def test_get_threshold_nfpa_dim001(
        self, repo
    ):  # function: def test_get_threshold_nfpa_dim001(self, repo):
        val, _, _ = repo.get_threshold("NFPA101-7.2.1.2", "civil", "NFPA 101-2021")  # 操作
        assert val == 1.12  # equality check

    def test_get_threshold_nfpa_dist001(
        self, repo
    ):  # function: def test_get_threshold_nfpa_dist001(self, repo):
        val, _, _ = repo.get_threshold("NFPA101-7.7.1", "civil", "NFPA 101-2021")  # 操作
        assert val == 61.0  # equality check

    def test_all_clauses_have_building_types(
        self, repo
    ):  # function: def test_all_clauses_have_building_types(self, repo):
    ...
    def test_l1_l2_l3_distribution(
        self, repo
    ):  # function: def test_l1_l2_l3_distribution(self, repo):
    ...
    def test_get_threshold_strict_single_type(
        self, repo
    ):  # function: def test_get_threshold_strict_single_type(self, repo):
        """单建筑类型：等同于 get_threshold"""
        effective_types = ["civil"]  # assignment

        def get_strict_threshold(
            clause_id: str,
        ):  # function: def get_strict_threshold(clause_id: str):
    ...
    def test_get_threshold_strict_mixed(
        self, repo
    ):  # function: def test_get_threshold_strict_mixed(self, repo):
        """混合建筑类型：取最严格（最大）阈值"""
    ...
    def test_get_threshold_strict_dist(
        self, repo
    ):  # function: def test_get_threshold_strict_dist(self, repo):
        """混合建筑类型：疏散距离场景"""
        effective_types = ["civil", "industrial"]  # assignment
...
class TestAttributionAnalyzer:  # class definition
    ...
    def make_result(
        ...
        func_id="DIM-001",
        result="FAIL",
        actual=1.05,  # function: def make_result(self, func_id="DIM-001", result="FAIL", actu
        threshold=1.2,
    ...
    def make_clause(self):  # function: def make_clause(self):
        return {
            ...
            "clause_id": "GB50016-5.5.18",  # return: dict
            "title": "疏散楼梯净宽",
            "text": "净宽度不应小于1.2m",  # 字段
    ...
    def test_finding_id_format(
        self, analyzer
    ):  # function: def test_finding_id_format(self, analyzer):
        f = analyzer.build_finding(
            self.make_result(), self.make_clause(), self.make_entity(), []
        )  # function call
        assert f.finding_id.startswith("BAA-")  # 断言

    def test_judgement_result(
        self, analyzer
    ):  # function: def test_judgement_result(self, analyzer):
        f = analyzer.build_finding(
            self.make_result(), self.make_clause(), self.make_entity(), []
        )  # function call
    ...
    def test_attention_map(self, analyzer):  # function: def test_attention_map(self, analyzer):
        f = analyzer.build_finding(
            self.make_result(),
            self.make_clause(),
            self.make_entity(),  # function call
            [{"id": "DR_007", "type": "door"}],
        )  # 字面量
    ...
    def test_explanation_not_empty(
        self, analyzer
    ):  # function: def test_explanation_not_empty(self, analyzer):
        f = analyzer.build_finding(
            self.make_result(), self.make_clause(), self.make_entity(), []
        )  # function call
        assert len(f.explanation) > 0  # 断言

    def test_suggestion_not_empty(
        self, analyzer
    ):  # function: def test_suggestion_not_empty(self, analyzer):
        f = analyzer.build_finding(
            self.make_result(), self.make_clause(), self.make_entity(), []
        )  # function call
        assert len(f.suggestion) > 0  # 断言

    def test_attention_map_has_heatmap(
        self, analyzer
    ):  # function: def test_attention_map_has_heatmap(self, analyzer):
        f = analyzer.build_finding(
            self.make_result(), self.make_clause(), self.make_entity(), []
        )  # function call
...
def test_synthetic_drawing_batch():  # function: def test_synthetic_drawing_batch():
    ...
    for entry in data["drawings"]:  # loop: iterate
        result = parser.parse(
            f"data/drawings/synthetic_v2/{entry['filename']}", entry["file_id"]
        )  # function call
        sem = analyzer.analyze(result.primitives)  # function call
        entities = sem["entities"]  # assignment
        expected_failed = {
            fid for fid, v in entry["violations"].items() if v["fail"]
        }  # function call
...
class TestDrawingParser:  # class definition
    ...
    def test_insert_block_expand_line(self):  # function: def test_insert_block_expand_line(self):
        ...
        parser._insert_block_expand(
            block_entities, msp, 200, 200, 2.0, 0, 1, "WALL"
        )  # function call
    ...
    def test_insert_block_expand_with_rotation(
        self,
    ):  # function: def test_insert_block_expand_with_rotation(self):
        """测试 INSERT 块展开：旋转 90°"""
        import ezdxf  # import
    ...
    def test_insert_block_expand_circle(
        self,
    ):  # function: def test_insert_block_expand_circle(self):
        ...
        parser._insert_block_expand(
            block_entities, msp, 100, 100, 3.0, 0, 1, "WALL"
        )  # function call
    ...
    def test_insert_block_expand_depth_limit(
        self,
    ):  # function: def test_insert_block_expand_depth_limit(self):
        """测试块嵌套展开深度限制（最深 5 层）"""
        import ezdxf  # import
...
class TestSemanticAnalyzer:  # class definition
    ...
    def test_merge_line_chains_empty_returns_entities(
        self,
    ):  # function: def test_merge_line_chains_empty_returns_entities(self):
        """无 LINE 图元时返回原实体列表"""
        analyzer = SemanticAnalyzer()  # function call
        entities = []  # assignment
    ...
    def test_merge_line_chains_too_few_returns_entities(
        self,
    ):  # function: def test_merge_line_chains_too_few_returns_entities(self):
        """LINE 数量 < 3 时返回原实体列表"""
        analyzer = SemanticAnalyzer()  # function call
        entities = []  # assignment
        primitives = [  # assignment
            RawPrimitive(
                ...
                "0",
                "h1",
                {"x": 0, "y": 0, "width": 1000, "height": 0},  # code
            ...
            RawPrimitive(
                ...
                "0",
                "h2",
                {"x": 1000, "y": 0, "width": 1000, "height": 1000},  # code
    ...
    def test_merge_line_chains_closed_square(
        self,
    ):  # function: def test_merge_line_chains_closed_square(self):
        ...
        primitives = [  # assignment
            RawPrimitive(
                ...
                "WALL",
                "h1",
                {"x": 0, "y": 0, "width": 10000, "height": 0},  # code
            ...
            RawPrimitive(
                ...
                "WALL",
                "h2",
                {"x": 10000, "y": 0, "width": 0, "height": 10000},  # code
            ...
            RawPrimitive(
                ...
                "WALL",
                "h3",
                {"x": 10000, "y": 10000, "width": 10000, "height": 0},  # code
            ...
            RawPrimitive(
                ...
                "WALL",
                "h4",
                {"x": 0, "y": 10000, "width": 0, "height": 10000},  # code
    ...
    def test_merge_line_chains_non_closed(
        self,
    ):  # function: def test_merge_line_chains_non_closed(self):
    ...
    def test_merge_line_chains_non_building_layer(
        self,
    ):  # function: def test_merge_line_chains_non_building_layer(self):
    ...
    def test_is_near_closed_gap_under_threshold(
        self,
    ):  # function: def test_is_near_closed_gap_under_threshold(self):
        """缺口距离 < 500mm → 视为闭合"""
        analyzer = SemanticAnalyzer()  # function call
        prim = RawPrimitive(
            "LWPOLYLINE",
            "WALL",
            "h1",
            {"x": 0, "y": 0, "width": 10000, "height": 10000},  # assignment
    ...
    def test_is_near_closed_gap_over_threshold(
        self,
    ):  # function: def test_is_near_closed_gap_over_threshold(self):
        """缺口距离 > 500mm → 不视为闭合"""
        analyzer = SemanticAnalyzer()  # function call
        prim = RawPrimitive(
            "LWPOLYLINE",
            ...
            {"x": 0, "y": 0, "width": 10000, "height": 10000},  # assignment
            {
                "area": 0,
                "point_count": 4,
                "points": [(0, 0), (10000, 0), (10000, 10000), (0, 9000)],
            },
        )  # function call
    ...
    def test_is_near_closed_few_points(self):  # function: def test_is_near_closed_few_points(self):
        ...
        prim = RawPrimitive(
            ...
            "WALL",
            "h1",
            {"x": 0, "y": 0, "width": 10000, "height": 0},  # assignment
            {"start_point": {"x": 0, "y": 0}, "end_point": {"x": 10000, "y": 0}},
        )  # literal: collection
        assert analyzer._is_near_closed(prim, gap_threshold_mm=500.0) is False  # function call

    def test_is_near_closed_malformed_pts(
        self,
    ):  # function: def test_is_near_closed_malformed_pts(self):
...
class TestFilterYOLODetections:  # class definition
    ...
    def test_corridor_width_filter(self):  # function: def test_corridor_width_filter(self):
        ...
        detections = [  # assignment
            {
                "type": "corridor",
                "bbox": {"x": 0, "y": 0, "width": 10, "height": 0.3},
                "confidence": 0.8,
                "properties": {},
            }  # literal: collection
        ]  # code
    ...
    def test_corridor_width_pass(self):  # function: def test_corridor_width_pass(self):
        ...
        detections = [  # assignment
            {
                "type": "corridor",
                "bbox": {"x": 0, "y": 0, "width": 10, "height": 0.8},
                "confidence": 0.8,
                "properties": {},
            }  # literal: collection
        ]  # code
    ...
    def test_door_near_wall_kept(self):  # function: def test_door_near_wall_kept(self):
        ...
        detections = [  # assignment
            {
                "type": "wall",
                "bbox": {"x": 0, "y": 0, "width": 10, "height": 0.2},
                "confidence": 0.9,
            },  # literal: collection
            {
                "type": "door",
                "bbox": {"x": 4, "y": 0, "width": 1, "height": 2},
                "confidence": 0.7,
            },  # literal: collection
        ]  # code
    ...
    def test_door_far_from_wall_suppressed(
        self,
    ):  # function: def test_door_far_from_wall_suppressed(self):
        """door 远离墙体应被过滤"""
        from src.baa_engine.yolo_integrator import filter_yolo_detections  # import
        ...
        detections = [  # assignment
            ...
            {
                "type": "door",
                "bbox": {"x": 50, "y": 50, "width": 1, "height": 2},
                "confidence": 0.7,
            },  # literal: collection
        ]  # code
        result = filter_yolo_detections(detections)  # function call
    ...
    def test_window_near_wall_kept(self):  # function: def test_window_near_wall_kept(self):
        ...
        detections = [  # assignment
            ...
            {
                "type": "window",
                "bbox": {"x": 3, "y": 0, "width": 2, "height": 0.5},
                "confidence": 0.8,
            },  # literal: collection
        ]  # code
        result = filter_yolo_detections(detections)  # function call
    ...
    def test_window_far_from_wall_suppressed(
        self,
    ):  # function: def test_window_far_from_wall_suppressed(self):
        """window 远离墙体应被过滤"""
        from src.baa_engine.yolo_integrator import filter_yolo_detections  # import
        ...
        detections = [  # assignment
            ...
            {
                "type": "window",
                "bbox": {"x": 100, "y": 100, "width": 2, "height": 1},
                "confidence": 0.8,
            },  # literal: collection
        ]  # code
        result = filter_yolo_detections(detections)  # function call
    ...
    def test_room_as_wall_segment_reference(
        self,
    ):  # function: def test_room_as_wall_segment_reference(self):
    ...
    def test_fire_door_same_as_door_rules(
        self,
    ):  # function: def test_fire_door_same_as_door_rules(self):
    ...
    def test_corridor_kept_with_adequate_width(
        self,
    ):  # function: def test_corridor_kept_with_adequate_width(self):
        ...
        detections = [  # assignment
            {
                "type": "corridor",
                "bbox": {"x": 0, "y": 0, "width": 20, "height": 2.0},
                "confidence": 0.6,
                "properties": {},
            },  # literal: collection
        ]  # code
...
class TestPersistentCache:  # class definition
    ...
    def test_get_nonexistent(
        self, tmp_cache
    ):  # function: def test_get_nonexistent(self, tmp_cache):
        """不存在的 key 返回 None"""
    ...
    def test_get_or_compute_hit(
        self, tmp_cache
    ):  # function: def test_get_or_compute_hit(self, tmp_cache):
        """缓存命中时不执行计算函数"""
    ...
    def test_get_or_compute_miss(
        self, tmp_cache
    ):  # function: def test_get_or_compute_miss(self, tmp_cache):
        """缓存未命中时执行计算函数并缓存"""
        compute_called = []  # assignment
...
class TestEvacuationConnectivity:  # class definition
    ...
    def test_verify_connectivity_room_with_exit(
        self,
    ):  # function: def test_verify_connectivity_room_with_exit(self):
        """room 通过走廊连接到 exit → 连通"""
        analyzer = SemanticAnalyzer()  # function call
        room = SemanticEntity(
            "ROOM_001",
            "room",
            {"x": 0, "y": 0, "width": 10, "height": 10},  # assignment
            "WALL",
            properties={"area": 100.0},
        )  # assignment
        corridor = SemanticEntity(
            "CORR_001",
            "corridor",
            {"x": 0, "y": 10, "width": 10, "height": 2},  # assignment
            "WALL",
            properties={"width": 2.0, "length": 10.0},
        )  # assignment
        exit_door = SemanticEntity(
            "EXIT_001",
            "exit",
            {"x": 0, "y": 12, "width": 2, "height": 2},  # assignment
            "DOOR",
        ...
        results = analyzer.verify_evacuation_connectivity(
            entities, relations, routes
        )  # function call
    ...
    def test_verify_connectivity_corridor_too_narrow(
        self,
    ):  # function: def test_verify_connectivity_corridor_too_narrow(self):
        """走廊宽度 < 1.2m → 标记瓶颈"""
        analyzer = SemanticAnalyzer()  # function call
        room = SemanticEntity(
            "ROOM_001",
        ...
        corridor = SemanticEntity(
            "CORR_001",
            "corridor",
            {"x": 0, "y": 10, "width": 10, "height": 1},  # assignment
            "WALL",
            properties={"width": 1.0, "length": 10.0},
        )  # assignment
        exit_door = SemanticEntity(
            "EXIT_001",
            "exit",
            {"x": 0, "y": 11, "width": 2, "height": 2},  # assignment
            "DOOR",
            properties={"width": 1.5},
        )  # assignment
        entities = [room, corridor, exit_door]  # assignment
    ...
    def test_verify_connectivity_no_route(
        self,
    ):  # function: def test_verify_connectivity_no_route(self):
        """无路径 → 未连通"""
        analyzer = SemanticAnalyzer()  # function call
        room = SemanticEntity(
            "ROOM_001",
    ...
    def test_verify_connectivity_room_not_in_routes(
        self,
    ):  # function: def test_verify_connectivity_room_not_in_routes(self):
        ...
        exit_ent = SemanticEntity(
            "EXIT_001",
            "exit",
            {"x": 5, "y": 5, "width": 2, "height": 2},  # assignment
            "DOOR",
            properties={"width": 1.5},
        )  # assignment
...
class TestEquipmentDetection:  # class definition
    ...
    def test_circle_sprinkler_on_fire_layer(
        self,
    ):  # function: def test_circle_sprinkler_on_fire_layer(self):
    ...
    def test_circle_sprinkler_on_fas_layer(
        self,
    ):  # function: def test_circle_sprinkler_on_fas_layer(self):
    ...
    def test_circle_equipment_on_elec_layer(
        self,
    ):  # function: def test_circle_equipment_on_elec_layer(self):
    ...
    def test_circle_evac_lighting_on_emergency_layer(
        self,
    ):  # function: def test_circle_evac_lighting_on_emergency_layer(self):
    ...
    def test_circle_large_radius_not_equipment(
        self,
    ):  # function: def test_circle_large_radius_not_equipment(self):
    ...
    def test_circle_no_fire_layer_default_column(
        self,
    ):  # function: def test_circle_no_fire_layer_default_column(self):
    ...
    def test_solid_on_fire_layer_sprinkler(
        self,
    ):  # function: def test_solid_on_fire_layer_sprinkler(self):
    ...
    def test_solid_on_elec_layer_equipment(
        self,
    ):  # function: def test_solid_on_elec_layer_equipment(self):
    ...
    def test_solid_non_fire_layer_other(
        self,
    ):  # function: def test_solid_non_fire_layer_other(self):
    ...
    def test_hatch_on_fire_layer_sprinkler(
        self,
    ):  # function: def test_hatch_on_fire_layer_sprinkler(self):
    ...
    def test_hatch_non_fire_layer_other(
        self,
    ):  # function: def test_hatch_non_fire_layer_other(self):
        """非消防图层上的 HATCH → other"""
        analyzer = SemanticAnalyzer()  # function call
        prim = RawPrimitive(
            "HATCH",
    ...
    def test_insert_smoke_detector_block(
        self,
    ):  # function: def test_insert_smoke_detector_block(self):
    ...
    def test_entity_type_enumeration(self):  # function: def test_entity_type_enumeration(self):
        ...
        device_types = [  # assignment
            ...
            "smoke_detector",
            "fire_alarm",
            "evacuation_lighting",  # code
            "emergency_broadcast",
            "fire_curtain",
            "equipment",  # code
            "water_tank",
            "water_reservoir",  # code
        ]  # code
...
class TestFloorDetection:  # class definition
    ...
    def _make_prim(
        self, dxf_type, layer, handle, bbox, props=None
    ):  # function: def _make_prim(self, dxf_type, layer, handle, bbox, props=No
        return RawPrimitive(dxf_type, layer, handle, bbox, props or {})  # return

    def test_no_floor_separators_returns_empty(
        self,
    ):  # function: def test_no_floor_separators_returns_empty(self):
        """无分隔线/标高文字时返回空列表"""
        analyzer = SemanticAnalyzer()  # function call
        prims = [  # assignment
            self._make_prim(
                "LINE", "WALL", "H001", {"x": 0, "y": 0, "width": 100, "height": 100}
            ),  # function call
        ]  # code
        result = analyzer._detect_floor_levels(prims)  # function call
        assert result == []  # equality check

    def test_horizontal_separator_detected(
        self,
    ):  # function: def test_horizontal_separator_detected(self):
        """跨越图纸宽度 80% 的水平线 → 识别为楼层分隔线"""
        analyzer = SemanticAnalyzer()  # function call
        prims = [  # assignment
            self._make_prim(
                "LINE", "WALL", "H001", {"x": 0, "y": 0, "width": 10000, "height": 10000}
            ),  # function call
            self._make_prim(
                "LINE", "WALL", "H002", {"x": 0, "y": 10000, "width": 10000, "height": 5}
            ),  # function call
            self._make_prim(
                "LINE", "WALL", "H003", {"x": 0, "y": 10000, "width": 10000, "height": 20000}
            ),  # function call
        ]  # code
        result = analyzer._detect_floor_levels(prims)  # function call
        assert len(result) >= 2  # get length
    ...
    def test_elevation_text_detected(self):  # function: def test_elevation_text_detected(self):
        ...
        prims = [  # assignment
            self._make_prim(
                ...
                "TEXT",
                "H001",
                {"x": 0, "y": 0, "width": 100, "height": 20},  # code
                {"text": "±0.000"},
            ),  # literal: collection
            self._make_prim(
                "TEXT",
                "TEXT",
                "H002",
                {"x": 0, "y": 5000, "width": 100, "height": 20},  # code
                {"text": "F2"},
            ),  # literal: collection
        ]  # code
        result = analyzer._detect_floor_levels(prims)  # function call
    ...
    def test_floor_label_text_detected(self):  # function: def test_floor_label_text_detected(self):
        ...
        prims = [  # assignment
            self._make_prim(
                ...
                "TEXT",
                "H001",
                {"x": 0, "y": 0, "width": 50, "height": 20},  # code
                {"text": "F1"},
            ),  # literal: collection
            self._make_prim(
                "TEXT",
                "TEXT",
                "H002",
                {"x": 0, "y": 10000, "width": 50, "height": 20},  # code
                {"text": "F2"},
            ),  # literal: collection
        ]  # code
        result = analyzer._detect_floor_levels(prims)  # function call
    ...
    def test_chinese_floor_label_detected(
        self,
    ):  # function: def test_chinese_floor_label_detected(self):
        """ "首层", "二层" 文字 → 识别为楼层"""
        analyzer = SemanticAnalyzer()  # function call
        prims = [  # assignment
            self._make_prim(
            ...
            self._make_prim(
                ...
                "TEXT",
                "H002",
                {"x": 0, "y": 10000, "width": 100, "height": 20},  # code
                {"text": "二层"},
            ),  # literal: collection
        ]  # code
        result = analyzer._detect_floor_levels(prims)  # function call
    ...
    def test_assign_entities_to_floors(self):  # function: def test_assign_entities_to_floors(self):
        ...
        room1 = SemanticEntity(
            "ROOM_001",
            "room",
            {"x": 100, "y": 100, "width": 1000, "height": 1000},  # assignment
            "WALL",
            properties={"area": 100.0},
        )  # assignment
        room2 = SemanticEntity(
            "ROOM_002",
            "room",
            {"x": 100, "y": 6000, "width": 1000, "height": 1000},  # assignment
            "WALL",
            properties={"area": 100.0},
        )  # assignment
        entities = [room1, room2]  # assignment
        assignments = analyzer._assign_entities_to_floors(
            entities, [], floor_levels
        )  # function call
    ...
    def test_assign_entity_outside_range(
        self,
    ):  # function: def test_assign_entity_outside_range(self):
        """超出所有楼层范围的实体分配到最近楼层"""
        analyzer = SemanticAnalyzer()  # function call
        floor_levels = [  # assignment
            {
        ...
        room = SemanticEntity(
            "ROOM_001",
            "room",
            {"x": 100, "y": -1000, "width": 1000, "height": 1000},  # assignment
            "WALL",
            properties={"area": 100.0},
        )  # assignment
    ...
    def test_analyze_includes_floor_info(
        self,
    ):  # function: def test_analyze_includes_floor_info(self):
        """analyze() 返回结果中包含楼层信息"""
        analyzer = SemanticAnalyzer()  # function call
        prims = [  # assignment
            self._make_prim(
                "LINE", "WALL", "H001", {"x": 0, "y": 0, "width": 10000, "height": 5000}
            ),  # function call
            self._make_prim(
                "LINE", "WALL", "H002", {"x": 0, "y": 5000, "width": 10000, "height": 5}
            ),  # function call
            self._make_prim(
                "LINE", "WALL", "H003", {"x": 0, "y": 5000, "width": 10000, "height": 10000}
            ),  # function call
        ]  # code
    ...
    def test_analyze_floor_property_on_entities(
        self,
    ):  # function: def test_analyze_floor_property_on_entities(self):
    ...
    def test_multiple_floors_with_separators(
        self,
    ):  # function: def test_multiple_floors_with_separators(self):
        """多条分隔线 → 3 个楼层"""
        analyzer = SemanticAnalyzer()  # function call
        prims = [  # assignment
            self._make_prim(
                "LINE", "WALL", "H001", {"x": 0, "y": 0, "width": 10000, "height": 3000}
            ),  # function call
            self._make_prim(
                "LINE", "WALL", "H002", {"x": 0, "y": 3000, "width": 9000, "height": 5}
            ),  # function call
            self._make_prim(
                "LINE", "WALL", "H003", {"x": 0, "y": 3000, "width": 10000, "height": 6000}
            ),  # function call
            self._make_prim(
                "LINE", "WALL", "H004", {"x": 0, "y": 6000, "width": 9000, "height": 5}
            ),  # function call
            self._make_prim(
                "LINE", "WALL", "H005", {"x": 0, "y": 6000, "width": 10000, "height": 9000}
            ),  # function call
        ]  # code
        result = analyzer._detect_floor_levels(prims)  # function call
...
class TestReviewScoring:  # class definition
    ...
    def test_confidence_perfect(self):  # function: def test_confidence_perfect(self):
        ...
        entity = {
            "type": "staircase",
            "properties": {"width": 1.5, "height": 3.0, "floor": "F1"},
        }  # assignment
        result = func.execute(entity)  # function call
        assert result is not None  # code
    ...
    def test_confidence_yolo_detected(self):  # function: def test_confidence_yolo_detected(self):
        ...
        entity = {
            "type": "staircase",
            "properties": {"width": 1.5, "detection_source": "yolo", "floor": "F1"},
        }  # assignment
        result = func.execute(entity)  # function call
        assert result is not None  # code
    ...
    def test_confidence_text_detected(self):  # function: def test_confidence_text_detected(self):
        ...
        entity = {
            "type": "staircase",
            "properties": {"width": 1.5, "detection_source": "text", "floor": "F1"},
        }  # assignment
        result = func.execute(entity)  # function call
        assert result is not None  # code
        assert result.confidence == 0.8  # equality check

    def test_confidence_missing_properties(
        self,
    ):  # function: def test_confidence_missing_properties(self):
        """缺少关键属性 → 置信度降低"""
        func = FuncRegistry().get("DIM-001")  # function call
        entity = {
            "type": "staircase",
            "properties": {"width": 1.5, "height": None, "area": None},
        }  # assignment
        result = func.execute(entity)  # function call
        assert result is not None  # code
    ...
    def test_confidence_no_floor_property(
        self,
    ):  # function: def test_confidence_no_floor_property(self):
        """无 floor 属性 → 置信度略微降低"""
        func = FuncRegistry().get("DIM-001")  # function call
        entity = {"type": "staircase", "properties": {"width": 1.5}}  # assignment
        result = func.execute(entity)  # function call
    ...
    def test_confidence_floor_property_present(
        self,
    ):  # function: def test_confidence_floor_property_present(self):
        """有 floor 属性 → 不降权"""
        func = FuncRegistry().get("DIM-001")  # function call
