...
var _mp_loading = false;

function switchModelParamTab(tab) {
  if (_mp_loading && tab !== 'export') return;
  _mp_loading = true;
...
function _mpSetHTML(box, html) {
  if (!box) { _mp_loading = false; return; }
...
function _mpSetError(box, err) {
  ...
  _mpSetHTML(box, '<p class="text-red-400 text-sm">加载失败: ' + msg + '</p>');
}
...
async function _mpLoadFunctions() {
  ...
  try {
    ...
    if (funcs.length === 0) { _mpSetHTML(box, '<p class="text-gray-400 text-sm">暂无数据</p>'); return; }

    var cats = {};
    funcs.forEach(function(f) { var c = f.category || 'other'; cats[c] = (cats[c] || 0) + 1; });
    ...
    var rows = funcs.slice(0, 80).map(function(f) {
      return '<tr><td class="py-1 px-2 text-xs text-mono">' + _mpEsc(f.func_id) + '</td>' +
        '<td class="py-1 px-2 text-xs">' + _mpEsc(f.title) + '</td>' +
        '<td class="py-1 px-2 text-xs"><span class="badge badge-xs">' + _mpEsc(f.category) + '</span></td>' +
        '<td class="py-1 px-2 text-xs text-mono">' + _mpEsc(f.clause_id) + '</td>' +
        '<td class="py-1 px-2 text-xs">' + _mpEsc(f.operator) + ' ' + _mpEsc(f.threshold) + '</td></tr>';
    }).join('');
...
async function _mpLoadLayerRules() {
  ...
  try {
    ...
    if (rules.length === 0) { _mpSetHTML(box, '<p class="text-gray-400 text-sm">暂无数据</p>'); return; }
    ...
    var rows = rules.slice(0, 60).map(function(r) {
      ...
      return '<tr><td class="py-1 px-2 text-xs text-mono">' + _mpEsc(r.pattern) + '</td>' +
        '<td class="py-1 px-2 text-xs">' + _mpEsc(r.entity_type) + '</td>' +
        '<td class="py-1 px-2 text-xs"><span class="badge badge-xs ' + cls + '">' + _mpEsc(r.source) + '</span></td>' +
        '<td class="py-1 px-2 text-xs">' + _mpEsc(r.match_type) + '</td></tr>';
    }).join('');
...
async function _mpLoadCDItems() {
  ...
  try {
    ...
    if (items.length === 0) { _mpSetHTML(box, '<p class="text-gray-400 text-sm">暂无数据</p>'); return; }
    ...
    var rows = items.slice(0, 50).map(function(i) {
      return '<tr><td class="py-1 px-2 text-xs text-mono">' + _mpEsc(i.item_id) + '</td>' +
        '<td class="py-1 px-2 text-xs">' + _mpEsc(i.title) + '</td>' +
        '<td class="py-1 px-2 text-xs"><span class="badge badge-xs">' + _mpEsc(i.level) + '</span></td>' +
        '<td class="py-1 px-2 text-xs">' + _mpEsc(i.major) + '</td></tr>';
    }).join('');
...
async function _mpLoadSamples() {
  ...
  try {
    ...
    if (samples.length === 0) {
      _mpSetHTML(box, '<p class="text-yellow-500 text-sm">暂无样本数据（需有审查记录后自动生成）</p>');
      return;
    }
    var cards = samples.slice(0, 10).map(function(s) {
      return '<div class="card p-2 mb-2">' +
        '<div class="text-xs text-gray-400">' + _mpEsc(s.created_at) + '</div>' +
        '<div class="text-sm font-medium">' + _mpEsc(s.title || s.func_id || '样本') + '</div>' +
        '<div class="text-xs text-mono">' + _mpEsc(s.func_id) + ' | ' + _mpEsc(s.dxf_file) + '</div>' +
        '<div class="text-xs text-gray-500 mt-1">' + (s.query ? _mpEsc(s.query).slice(0, 200) : '') + '</div>' +
      '</div>';
    }).join('');
