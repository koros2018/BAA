...
const P119Audit = (function () {
  ...
  function api(path, options = {}) {
    const url = window.BAA_API_BASE + path;
    const opts = {
      method: "GET",
    ...
    return fetch(url, opts).then(function (r) {
      if (!r.ok) return r.json().then(function (d) { throw Object.assign(new Error(d.detail || "API error"), { status: r.status, detail: d.detail }); });
      return r.json();
    });
  }
  ...
  function refreshStats() {
    return api("/api/v1/audit/stats?review_id=" + encodeURIComponent(_state.reviewId)).then(function (d) {
  ...
  function _renderStatsBar() {
    var bar = document.getElementById("p119-stats-bar");
    if (!bar) return;
    var s = _state.stats;
    var done = s.confirmed + s.dismissed;
    bar.innerHTML =
      ...
      "</select>" +
      "</div>";
    var sel = document.getElementById("p119-filter");
    if (sel) {
  ...
  function _renderCompletedButton() {
    var holder = document.getElementById("p119-completed-holder");
    if (!holder) return;
    holder.innerHTML = "";
    var s = _state.stats;
    if (s.total > 0 && s.unreviewed === 0) {
      ...
      btn.onclick = function () {
        showToast("整改通知单生成功能（P116）开发中", "info");
      };
  ...
  function _refreshItems() {
    api(
      "/api/v1/audit/items?review_id=" + encodeURIComponent(_state.reviewId) +
        (_state.filter ? "&status=" + encodeURIComponent(_state.filter) : "")
    )
      .then(function (d) {
        var items = d.items || [];
        var bar = document.getElementById("p119-stats-bar");
        if (bar && bar.dataset.itemsSlot) {
          var slot = bar.parentElement.querySelector(".p119-items-list");
          if (slot) {
            if (items.length === 0) {
            ...
            slot.innerHTML = items
              .map(function (it) {
                var cls = "p119-item p119-item-" + it.status;
                var statusLabel = {
                  unreviewed: "未审核",
                  confirmed: "✓ 已确认",
                  dismissed: "✗ 已驳回",
              ...
              .join("");
            slot.querySelectorAll(".p119-act").forEach(function (btn) {
              btn.onclick = function () {
  ...
  function _handleAction(itemId, action) {
    if (action === "confirm") {
      api("/api/v1/audit/items/" + encodeURIComponent(itemId) + "/confirm", {
        method: "POST",
        body: JSON.stringify({ note: "" }),
      }).then(function () { refreshStats(); _refreshItems(); showToast("已确认违规", "success"); });
    } else if (action === "dismiss") {
      var reason = prompt("误报原因（必填）：", "实测宽度合格，图纸尺寸有误差");
      if (!reason) return;
      api("/api/v1/audit/items/" + encodeURIComponent(itemId) + "/dismiss", {
        method: "POST",
        body: JSON.stringify({ reason: reason }),
      }).then(function () { refreshStats(); _refreshItems(); showToast("已标记误报，已自动写入反馈库", "success"); });
    } else if (action === "pending") {
      api("/api/v1/audit/items/" + encodeURIComponent(itemId) + "/pending", {
        method: "POST",
        body: JSON.stringify({ note: "待现场复核" }),
      }).then(function () { refreshStats(); _refreshItems(); showToast("已标记待核实", "info"); });
    } else if (action === "note") {
      var note = prompt("批注内容：", "");
      if (!note) return;
      api("/api/v1/audit/items/" + encodeURIComponent(itemId) + "/note", {
        method: "PATCH",
        body: JSON.stringify({ note: note }),
      }).then(function () { refreshStats(); _refreshItems(); showToast("批注已更新", "success"); });
    }
  }

  // ── 对外接口 ──────────────────────────────────────────

  function init(containerSelector, reviewId, details) {
    _state.reviewId = reviewId;
    var container = document.querySelector(containerSelector) || document.getElementById("p119-audit-container");
    if (!container) {
    ...
    if (details && details.length > 0) {
      api("/api/v1/audit/items", {
        ...
        console.warn("[P119Audit] 初始化失败:", e);
      });
    } else {
    ...
    _injectStyles();
    return { refreshStats: refreshStats, setReviewId: setReviewId };
  }

  function setReviewId(rid, details) {
    _state.reviewId = rid;
    if (details) {
      api("/api/v1/audit/items", {
        method: "POST",
        body: JSON.stringify({ review_id: rid, details: details }),
      }).then(function () { refreshStats(); _refreshItems(); });
    } else {
      refreshStats().catch(function () {});
      _refreshItems();
    }
  ...
  return { init: init, setReviewId: setReviewId, refreshStats: refreshStats };
})();